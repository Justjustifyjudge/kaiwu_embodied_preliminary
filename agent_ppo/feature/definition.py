...
def reward_process(end_dist, history_dist, nearest_treasure_dist, used_flash=False):
    """改进的奖励函数，包含宝箱距离奖励和闪现惩罚"""
    # 基础步数惩罚
    step_reward = -0.001
    
    # 终点距离奖励
    end_reward = -0.05 * end_dist
    
    # 移动进步奖励
    dist_reward = min(0.001, 0.1 * history_dist)
    
    # 宝箱距离奖励（反比例函数）
    if nearest_treasure_dist > 0:
        treasure_reward = Config.TREASURE_BASE_REWARD / (nearest_treasure_dist + 1)
    else:
        treasure_reward = 0
    
    # 闪现使用惩罚（防止滥用）
    flash_penalty = -0.2 if used_flash else 0
    
    return [step_reward + dist_reward + end_reward + treasure_reward + flash_penalty]
